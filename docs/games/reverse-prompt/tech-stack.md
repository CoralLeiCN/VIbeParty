...
| Video generation | Reactor FastH3 (`reactor/fast-h3`), locked `reactor-sdk` 1.5.1 | Three independent text-only clips in one retained session |
...
Use fixed model `reactor/fast-h3` and one creator-owned session per round, retained through the two 30-second relay turns. The input is that prompt plus the existing fixed instruction: “A single continuous shot depicting the described scene. No captions or on-screen text.” Keep the original player text separately for scoring. Each enqueue contains only the current prompt and fixed instruction. Omit `continue_from_clip_id` and `starting_frame`; no previous prompt, image or video is supplied as conditioning. Consume only `main_video`; the game's saved output remains silent even though FastH3 also generates audio. [FastH3 overview](https://docs.reactor.inc/model-api-reference/fast-h3/overview).
...
1. Persist one consumed attempt and set the unresolved-session guard before contacting Reactor. Mint one token restricted to `reactor/fast-h3`, `max_sessions=1`, and `max_session_duration_seconds=360`. Retain the 90-second deadline per generation covering connection when needed, generation, capture, encoding and validation. A 345-second application watchdog bounds the round from A's accepted submission. Each later accepted relay prompt consumes another attempt before its enqueue.
2. Confirm acknowledgments for `set_autoplay(false)`, `set_canvas` with aspect `16:9`, and `set_flush_on_clip_end(true)` once per session. Enqueue the current text plus fixed instruction with opaque attempt metadata and `seconds=5.167`. Require a correlated `clip_queued` identity, a matching completion event and a declared 124-frame source length. A compact completion event may omit that length: preserve acknowledged metadata or make one read-only lookup of the same playout item; never enqueue again. A differing duration fails this selected contract. The command/event definitions are documented in the [FastH3 schema](https://docs.reactor.inc/model-api-reference/fast-h3/schema).
...
5. Stop playback after each capture, retire its clip ID and validate the MP4 with ffprobe and bounded decoding within the remaining deadline. Keep the session open after V0 and V1 while the next interpreter watches and submits within 30 seconds. Publish only that interpreter's authorized clue; refresh does not reset the server deadline. After V2, or on error, expiry, reset, party closure or shutdown, terminate the session and independently verify terminal state before clearing the guard. Final guessing begins only after confirmed closure. Keep encoder cancellation owned and kill/await it when required. The provider duration cap remains a crash fallback.

Existing FFmpeg tests establish encoding and frame order for supplied frames, and the prior provider fakes cover independence, quota and cleanup. Current tests cover clip acknowledgment, generation, explicit playback, frame admission, failed/short playback, three clips in one session and grouped quota closure. Live timing and phone compatibility remain open. See the [game handoff](../../development/handoffs/reverse-prompt.md), [Python SDK](https://docs.reactor.inc/sdk-reference/python/reactor) and [token authentication](https://docs.reactor.inc/authentication).

**Use one active provider session, one attempt per step, and no automatic session-creation retries.** The initial campaign was explicitly initialized with nine attempts and no unresolved session. Its original ledger must be restored to durable storage before live use; do not initialize a new allowance to replace the missing record. Decrement atomically before each accepted prompt starts provider work, never refund ambiguous attempts, and never replenish on room reset or process restart. Up to three attempt records share a `session_attempt` group and session ID; terminal verification closes the entire group atomically. A new process or unrelated session cannot consume while the group is unresolved. A missing/corrupt guard file blocks live mode until the operator restores or repairs the existing campaign record. Do not bake automatic quota initialization into startup.
