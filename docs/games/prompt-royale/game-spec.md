...
| Name | Prefill host and guest fields with an editable random valid name; preserve edits through form changes and errors. Accept 1–24 Unicode code points after trimming and NFC normalization; suffix duplicate names. |
...
Start another round clears the finished round's prompts, ballots, clips, and topic selection/confirmation and returns the same players to the two topic modes. Any new LLM suggestion requires fresh host confirmation. **End game** asks for confirmation, requests party closure, and returns the host to the portal to track any pending cleanup. Completed closure removes the room and its media. A new round always has a fresh ID and new ballots.
...
| --- | --- |
| Full round | Three browsers complete join → prompt → generate → screen → vote → results, then Start another round. |
