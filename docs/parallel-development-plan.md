...
> Implement Reverse Prompt locally on the host laptop, end to end in your assigned directories. Read its current game/technical specs and `docs/parallel-development-plan.md`. Prove independent FastH3 capture and offline scoring, then build the three-player relay and live integration. Preserve private clue access including media range requests, 30-second B/C relay turns with untimed author input and final guesses, one retained session for three independent clips, local MiniLM scoring, unscored failure behavior, and the persistent quota/closure guard. Own DEMO-01 through DEMO-08 verification and your browser rehearsal. Request shared dependencies/model setup from A and record evidence/milestone SHAs. Coordinate every live trial with A.
