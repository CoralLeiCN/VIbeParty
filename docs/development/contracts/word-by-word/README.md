...
| POST `/join` | `{code,name}`. 3–4 player roster, join order preserved. Existing player cookie resumes. |
| GET `/state` | Explicit role projection below. Does not update inactivity. |
| POST `/round/start` | `{round_id,mode:"fixture"}` or live after the live gate. Freezes roster, four round-robin slots, 45-second collection. Duplicate/stale start conflicts. |
...
| POST `/round/new` | `{round_id}`. RESULTS only; new ID, cleared files/text, same roster order. Blocks active generation/unconfirmed closure. |
| POST `/room/reset` | `{round_id}`. LOBBY/RESULTS; clears roster/replay, rotates resolver code atomically, keeps host cookie. Same guards. |
