...
Word by Word host cookies have a seven-day Max-Age to survive browser restart; server-side party expiry still applies. Player cookies remain session cookies. A missing/invalid host cookie does not authorize host creation or closure. Explicit recovery is available from the portal switch dialog and Word by Word host entry. In passcode mode it requires the configured host passcode; local mode requires the party's random recovery code, stored only in the private `<MEDIA_ROOT>/word-by-word/host-recovery.txt` file (0600). Run `uv run python -m scripts.host_recovery` on the host laptop, from the running project with matching settings, to read it. The code survives round/reset cleanup and is removed on party close/startup. No HTTP endpoint returns the code.

GET `/api/games/word-by-word/host/recovery` returns `{available:false}` or `{available:true,party_id,method:"local_code"|"passcode"}`. The opaque public party ID is a stale-request guard, not ownership proof. POST to the same path with `{party_id,credential}` checks Origin, limits attempts per client and globally (20/minute), checks the exact party and proof under the lifecycle/room locks, rotates the host token and sets its cookie. Old host sessions are revoked; players, round data, generation limits and pending provider cleanup remain. Recovery returns `{status:"recovered"}` and never closes or switches automatically. The portal retains the chosen destination for the subsequent explicit close confirmation.
