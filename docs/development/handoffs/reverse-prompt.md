...
1. Wait for A’s separate allocation for the three-attempt full live relay and one deliberately failed generation. The initial one-attempt slot has been completed and returned; unused campaign attempts are not automatic authorization to spend. Word’s three earlier sessions are independently confirmed closed.
2. Run A→B→C→both guesses→local scoring→ordered reveal over HTTP. Verify private live media/range access, recovery and duplicate protection. Use the same canonical campaign path; never replenish it on reset/restart.
3. Complete the separately allocated forced-failure trial and provider-cap behavior, retaining the unresolved marker whenever closure cannot be proved.
4. Run the three-person event-device rehearsal and physical-phone playback/replay. Three browser identities on one laptop do not establish physical-device acceptance.

DEMO-02/07/08 are not fully passed yet. DEMO-01/03/04/05/06 retain local browser, HTTP or offline evidence; their full-live/device repeats remain pending.
