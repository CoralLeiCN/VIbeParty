...
- [x] Review and integrate the game-owned FastH3 replacement: enqueue a clip, wait for generation, explicitly play it, and capture a private MP4. Preserve current-prompt-only input, role privacy, attempt accounting and cleanup guards. The later user-directed change retains one session through three independent clips and both 30-second relay turns; local provider and browser checks pass.
...
- [x] Initialize one nine-attempt campaign at `/Users/coral/.codex/worktrees/5a4f/VIbeParty/.local/vibeparty-persistent/reverse-prompt/quota.json`. That integration worktree has since been removed and the original ledger is missing. Never initialize another campaign or replenish this one. Slot `2026-09-12-reverse-001` consumed one attempt and is closed; a later first relay generation consumed one more attempt. The last independently verified state was seven remaining, no unresolved session and no further allocation.
...
- [ ] Restore the original campaign record to durable storage and configure `REVERSE_PROMPT_QUOTA_FILE` there before enabling live mode.
- [ ] Complete a full live relay using one retained session and three independent clips, each receiving only the current player's prompt; verify both 30-second relay deadlines. Check that B and C see only their permitted clue at each step.
